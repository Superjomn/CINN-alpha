...
#include "cinn/model_loader/fluid/op_desc.h"

namespace cinn {
namespace model_loader {
namespace fluid {

google::protobuf::internal::RepeatedPtrIterator<framework::proto::OpDesc_Attr> FindAttr(framework::proto::OpDesc *desc,
  ...
  if (it == xs.end()) {
    auto *attr = xs.Add();
    attr->set_name(name);
    it = std::find_if(xs.begin(), xs.end(), [&](const framework::proto::OpDesc_Attr &x) { return x.name() == name; });
  }
  return it;
}
...
google::protobuf::internal::RepeatedPtrIterator<const framework::proto::OpDesc_Attr> GetFindAttr(
    const framework::proto::OpDesc &desc, const std::string &name) {
  auto &xs = desc.attrs();
  auto it =
      std::find_if(xs.begin(), xs.end(), [&](const framework::proto::OpDesc_Attr &x) { return x.name() == name; });
  return it;

...
struct variant {
 ...
 public:
  ...
  variant(const variant<Ts...>& old) : type_id(old.type_id) { helper_t::copy(old.type_id, &old.data, &data); }
  variant(variant<Ts...>&& old) : type_id(old.type_id) { helper_t::move(old.type_id, &old.data, &data); }
  ...
  const T& get() const {
    ...
    if (type_id == typeid(T).hash_code()) {
      ...
      LOG(FATAL) << "unmatched type, store as " << type_id << " , but want to get " << typeid(T).name();
#else
      throw std::invalid_argument(
          cinn::StringFormat("unmatched type, store as %d, but want to get %s", type_id, typeid(T).name()));
#endif
    }
    return *reinterpret_cast<const T*>(&data);
  }
...
#ifdef LITE_ON_TINY_PUBLISH
      LOG(ERROR) << "unmatched type get, should be " << type_id << " but get " << typeid(T).name();
#else
